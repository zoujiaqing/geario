...
pub use crate::error::utils::{ResultSignature, Retryable, Success, with_service};

#[doc(hidden)]
pub use crate::error::bt::{set_backtrace_start, set_backtrace_start_alt};
    ...
    async fn test_error() {
        ...
        #[cfg(unix)]
        {
            ...
            if let Some(bt) = err.backtrace() {
                bt.resolver().resolve();
                assert!(
                    format!("{bt}").contains("geario::error::tests::test_error"),
                    "{bt}",
                );
                assert!(
