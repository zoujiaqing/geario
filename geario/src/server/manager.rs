...
impl<F: ServerConfiguration> ServerManager<F> {
    pub(crate) fn start(cfg: WorkerPool, factory: F) -> Server<F::Item> {
        ...
        // handle cmd
        crate::rt::spawn(handle_cmd(mgr.clone(), rx));
        ...
        if no_signals {
            ...
            crate::rt::spawn(async move {
                while let Ok(sigs) = crate::rt::signals::signal().await {
...
impl<F: ServerConfiguration> HandleCmdState<F> {
    ...
    async fn stop(&mut self, graceful: bool, completion: Option<oneshot::Sender<()>>) {
        ...
        if self.mgr.0.cfg.stop_runtime {
            log::trace!("Stopping geario system, {:?} server", self.mgr.0.cfg.name);
            System::current().stop();
        }
    ...
    loop {
        ...
        match item {
            ...
            ServerCommand::Signal(sig) => {
                // Signals support
                // Handle `SIGINT`, `SIGTERM`, `SIGQUIT` signals and stop geario system
                match sig {
                    Signal::Int => {
