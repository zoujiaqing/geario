...
macro_rules! syscall {
    ...
    ($e:expr, $f:ident($fd:expr)) => {
        match $crate::syscall!(break $e) {
            ::std::task::Poll::Pending => Ok($crate::rt::sys::Decision::$f($fd)),
            ::std::task::Poll::Ready(Ok(res)) => Ok($crate::rt::sys::Decision::Completed(res)),
...
pub(crate) fn block_on<F, R>(run: &dyn Runner, fut: F) -> Result<R, Box<dyn Any + Send>>
where
    ...
    R: 'static,
{
    ...
    crate::error::set_backtrace_start_alt("geario/src/rt/driver.rs", 0);
...
pub(crate) fn block_on_panic<F, R>(run: &dyn Runner, fut: F) -> R
where
    F: Future<Output = R> + 'static,
    R: 'static,
{
