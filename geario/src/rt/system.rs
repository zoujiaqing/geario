...
use crate::rt::arbiter::Arbiter;
use crate::rt::pool::ThreadPool;
use crate::rt::{BlockingResult, Builder, Handle, HashMap, HashSet, Runner, SystemRunner};
...
pub struct SystemConfig {
    ...
    pub(crate) ping_interval: usize,
    #[allow(dead_code)]
    pub(crate) ping_threshold: usize,
    pub(crate) pool_limit: usize,
    pub(crate) pool_recv_timeout: Duration,
...
impl System {
    ...
    pub(crate) fn start(config: SystemConfig) -> (Self, oneshot::Receiver<i32>) {
        ...
        crate::rt::spawn(SystemSupport::new(&sys, stop_tx).run());
        crate::rt::spawn(controller.run(sys.clone()));
    ...
    pub fn set_latency_callback<F: Fn(crate::error::Backtrace) + 'static>(f: F) {
        unsafe {
...
impl SystemSupport {
    ...
    async fn run(mut self) {
        if self.sys.0.config.ping_interval != 0 {
            crate::rt::spawn(ping_arbiters(self.sys.clone()));
        }
...
async fn ping_arbiters(sys: System) {
    ...
    loop {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if !crate::rt::signals::is_enabled() {
                continue;
            }
                ...
                if result == -1 {
                    ...
                    for _ in 0..1000 {
                        ...
                        if let Some(bt) = CAPTURED.lock().take() {
                            let bt = crate::error::Backtrace::from(bt);
...
#[cfg(target_os = "linux")]
static mut ARB_CB: Option<Box<dyn Fn(crate::error::Backtrace)>> = None;

#[cfg(target_os = "linux")]
static EXPECTED_TID: std::sync::atomic::AtomicI32 = std::sync::atomic::AtomicI32::new(0);
#[cfg(target_os = "linux")]
static CAPTURED: Mutex<Option<crate::error::BacktraceRaw>> = Mutex::new(None);
...
pub(crate) fn sig_usr2() {
    ...
    #[allow(clippy::cast_possible_truncation)]
    {
        ...
        if EXPECTED_TID.load(Ordering::Acquire) == tid {
            ...
            let bt = crate::error::BacktraceRaw::new(panic::Location::caller());
