...
use crate::bytes::{BufMut, BytePageSize, ByteString, Bytes, BytesMut};
use crate::bytes::{buf::UninitSlice, stvec::StorageVec};
...
impl fmt::Debug for BytePages {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(st) = &self.current {
            f.field(&crate::bytes::debug::BsDebug(st.as_ref()));
        }
        f.finish()
    }
}
...
impl fmt::Debug for BytePage {
    fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Debug::fmt(&crate::bytes::debug::BsDebug(self.as_ref()), fmt)
    }
}
...
mod tests {
    ...
    fn pages_methods() {
        ...
        let mut pages = BytePages::default();
        pages.put_slice(b"456");
        pages.prepend(BytePage::from(crate::bytes::ByteString::from_static("123")));
        let mut pages2 = BytePages::default();
