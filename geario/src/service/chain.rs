...
use crate::service::and_then::{AndThen, AndThenFactory};
use crate::service::apply::{Apply, ApplyCtx, ApplyFactory};
use crate::service::ctx::Ctx;
use crate::service::fn_ready::FnReadiness;
use crate::service::fn_shutdown::FnShutdown;
use crate::service::map::{Map, MapFactory};
use crate::service::map_err::{MapErr, MapErrFactory};
...
impl<S: Service<St, Req>, St, Req> ServiceChain<S, St, Req> {
    ...
    where
        ...
        Err: From<S::Error>,
    {
        crate::service::apply_fn(self.service, f)
    }
...
impl<Sf: ServiceFactory<St, Req, Cfg>, St, Req, Cfg> ServiceChainFactory<Sf, St, Req, Cfg> {
    ...
    {
        crate::service::apply(tr, self.factory)
    }
