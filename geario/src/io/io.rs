...
use crate::bytes::{BytePageSize, BytesMut};
use crate::codec::{Decoder, Encoder};
use crate::service::cfg::{Cfg, SharedCfg};
use crate::util::{future::Either, task::LocalWaker, time::Sleep};

use crate::io::buf::Stack;
use crate::io::cfg::IoConfig;
use crate::io::ctx::IoContext;
use crate::io::filter::{Base, Filter, Layer};
use crate::io::filterptr::FilterPtr;
use crate::io::flags::Flags;
use crate::io::ops::{Id, IoManager, TimerHandle};
use crate::io::seal::{IoBoxed, Sealed};
use crate::io::utils::Extensions;
use crate::io::{Decoded, FilterLayer, Handle, IoStatusUpdate, IoStream, RecvError};

/// Interface object to the underlying I/O stream
pub struct Io<F = Base>(UnsafeCell<IoRef>, marker::PhantomData<F>);

#[derive(Clone)]
pub struct IoRef(pub(crate) Rc<IoState>);

pub(crate) struct IoState {
    filter: FilterPtr,
    pub(crate) id: Cell<Id>,
    pub(crate) cfg: Cfg<IoConfig>,
    pub(crate) flags: Flags,
    pub(crate) error: Cell<Option<io::Error>>,
    pub(crate) read_task: LocalWaker,
    pub(crate) write_task: LocalWaker,
    dispatch_task: LocalWaker,
    pub(crate) buffer: Stack,
    pub(crate) handle: Cell<Option<Box<dyn Handle>>>,
    pub(crate) timeout: Cell<TimerHandle>,
    pub(crate) shutdown_timeout: Cell<Option<Sleep>>,
...
impl IoState {
    ...
    pub(crate) fn error_or_disconnected(&self) -> io::Error {
        self.error()
            .unwrap_or_else(|| io::Error::new(io::ErrorKind::NotConnected, "Disconnected"))
    }

    pub(crate) fn filters_stopped(&self) {
        self.wake_read_task();
    ...
    pub(crate) fn terminate_connection(&self, err: Option<io::Error>) {
    ...
    pub(crate) fn get_read_buf(&self) -> BytesMut {
        self.cfg.read_buf().get()
    }

    pub(crate) fn is_rd_backpressure_needed(&self, size: usize) -> bool {
        size >= self.cfg.read_buf().high
    }

    pub(crate) fn is_wr_backpressure_needed(&self, size: usize) -> bool {
        size >= self.cfg.write_buf().high
    }

    pub(crate) fn should_disable_wr_backpressure(&self, size: usize) -> bool {
        size <= self.cfg.write_buf().half
    }

    pub(crate) fn wake_read_task(&self) {
        self.read_task.wake();
    }

    pub(crate) fn wake_write_task(&self) {
        #[cfg(feature = "trace")]
        log::trace!("{}: Wake write task, flags:{:?}", self.tag(), self.flags);
        self.write_task.wake();
    }

    pub(crate) fn wake_dispatch_task(&self) {
...
impl OnDisconnect {
    pub(crate) fn new(inner: Rc<IoState>) -> Self {
...
#[cfg(test)]
mod tests {
    use crate::bytes::{BufMut, BytePages, Bytes, BytesMut};
    use crate::codec::BytesCodec;
    use crate::util::{future::lazy, time::Millis, time::sleep};

    use super::*;
    use crate::io::{FilterBuf, IoContext, IoTaskStatus, Readiness, ops::Iops, testing::IoTest};
    ...
    async fn test_read() {
        ...
        let fut = crate::rt::spawn(async move {
            let mut buf: [u8; 4] = [0, 0, 0, 0];
    ...
    async fn shutdown_flushes_write_buf_with_read_backpressure() {
        ...
        let io = Io::new(
            ...
            SharedCfg::new("SRV").add(
                IoConfig::default()
                    .set_read_buf(8, 4, 16)
                    .set_disconnect_timeout(crate::util::time::Seconds(2)),
            ),
        );
        ...
        let data = crate::util::time::timeout(Millis(2000), client.read())
            .await
        ...
        crate::util::time::timeout(Millis(4000), io.on_disconnect())
            .await
