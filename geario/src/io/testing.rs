...
use crate::util::time::{Millis, sleep};
...
impl IoStream for IoTest {
    fn start(self, ctx: IoContext) -> Box<dyn Handle> {
        let io = Rc::new(self);
        crate::util::spawn(run(io.clone(), ctx));
        Box::new(io)
    }
}
...
mod tests {
    ...
    async fn basic() {
        ...
        let client = crate::io::Io::from(client.set_peer_addr(addr));
        let item = client.query::<crate::io::types::PeerAddr>();
