...
use crate::io::{Io, IoConfig, types};
use crate::service::{Ctx, Service, cfg::SharedCfg};
use crate::util::{future::Either, time::timeout_checked};
...
async fn connect<A: Address>(
    ...
    let io = match addr {
        Either::Left(addr) => crate::net::tcp_connect(addr, cfg.clone())
            .await
            .map_err(ConnectError::from)?,
        Either::Right(mut addrs) => loop {
            let addr = addrs.pop_front().unwrap();

            match crate::net::tcp_connect(addr, cfg.clone()).await {
                Ok(io) => break io,
                Err(err) => {
                    log::trace!(
