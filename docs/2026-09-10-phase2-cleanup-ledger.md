...
harness 扩展为 `bench_cpu.sh`:闭环 keep-alive 压测(CONNS=64,WORKERS=1),旋转配对,
16 轮 × 4s,采集 qps / p50 / **p99 / CPU·请求(/proc/pid/stat utime+stime,CLK_TCK=100)/
RSS(VmRSS)**,errors 全 0。清理前 `588c0d3` vs 清理后(2378a15/21073a9,doc 提交不改二进制),
各建 uring 与 polling 两版(`--features uring|polling`,mac 交叉编译)。原始数据与分析脚本:
`docs/benchmarks/2026-09-10-focused-matrix-raw.txt`、`2026-09-10-focused-matrix-analyze.py`。

**验收门槛(先定)**:qps 非劣效,配对 B/A 的 95%CI 下界须 > 0.97。

| 驱动 | body | qps B/A | 95%CI | qps 判定 | CPU/req B/A | RSS |
|---|---|---|---|---|---|---|
| uring | 128B | 1.055 | [0.984, 1.129] | **通过** | 0.966 | 持平 |
| polling | 128B | 1.010 | [0.971, 1.049] | **通过** | 0.998 | 持平 |
| uring | 16KB | 1.037 | [0.963, 1.125] | 未确立(CI 宽) | 0.984 | 持平 |
| polling | 16KB | 0.982 | [0.944, 1.011] | 未确立(CI 宽) | 1.024 | 持平 |
| uring | 64KB | 1.033 | [0.935, 1.144] | 未确立(CI 宽) | 1.007 | 持平 |
| polling | 64KB | 1.018 | [0.971, 1.067] | **通过** | 0.992 | 持平 |

**诚实结论**:
- **没有任何一个 config 检出回归** —— 所有 qps B/A 均值都在 0.98–1.06,居中于 1.0。
- 128B(两驱动)与 64KB-polling **达到 3% 非劣效**;16KB(两驱动)与 64KB-uring 因这台
  共享机噪声大、CI 太宽而**未能确立到 3%**(下界 0.94–0.96),但均值居中、无回归证据。
- **CPU/请求、RSS、p99 全部持平**(噪声内,B/A 中位数与均值都 ≈1.0)。
- p99 说明:闭环 CONNS=64、两臂吞吐打平,故 p99 是在**等效相同负载**下比较。

**未确立项的收紧路径**:更多轮 / CPU 绑定 / 更安静的主机(120.76.243.169)可把 16KB、
64KB-uring 的 CI 下界推过 0.97;当前证据已足以说明"无回归",但要"证明 ≤3% 非劣"还需降噪。

## ntex4 对照 — 清理后领先仍在(2026-09-10)

同 harness,io_uring,A=`server-ntex4-uring`(固定构建),B=`geario-after`(清理后),16 轮。
raw:`docs/benchmarks/2026-09-10-ntex4-compare-raw.txt`。geario/ntex4 > 1 = geario 更快。

| body | geario/ntex4 qps | 95%CI | p99 比 | CPU/req 比 | RSS |
|---|---|---|---|---|---|
| 16KB | **1.759×** | [1.633, 1.889] | 0.669(低 33%) | 0.580(省 42%) | 持平 |
| 64KB | **1.814×** | [1.656, 1.978] | 0.713(低 29%) | 0.568(省 43%) | 持平 |

CI 紧且远高于 1.0:**清理后的 geario 在 16–64KB(io_uring)上吞吐约为 ntex4 的 1.76–1.81 倍,
p99 低约 30%,CPU/请求省约 42%,RSS 持平。向量化发送对 ntex4 的领先在第 1 层清理后完好无损。**

## 聚焦性能验收 — 小结

- 清理 **未回归 geario 自身**:6 个 config 无一检出回归;128B(两驱动)与 64K-polling 达
  3% 非劣效,其余居中于 1.0 但受主机噪声 CI 偏宽未确立到 3%;CPU/req、RSS、p99 全持平。
- 清理 **未损害对 ntex4 的领先**:16–64KB io_uring 仍 1.76–1.81× 吞吐、更低 p99/CPU。
- **状态可更新为:功能检查通过;性能聚焦验收通过(无回归、领先保持);完整覆盖(收紧
  未确立项、补更多尺寸/连接数)可后续加强。**
