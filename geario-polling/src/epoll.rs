...
pub struct Poller {
    ...
    /// Whether the timerfd is currently armed, so a `wait` that asks for no
    /// timeout again does not have to disarm what is already disarmed.
    /// Almost every wait in a server asks for no timeout at all.
    ///
    /// Atomic because `Poller` is `Sync`; it is only ever written by the
    /// thread inside `wait`, so ordering beyond `Relaxed` buys nothing.
    #[cfg(not(target_os = "redox"))]
...
impl Poller {
    ...
    pub fn new() -> io::Result<Poller> {
        ...
        unsafe {
            ...
            if let Some(ref timer_fd) = poller.timer_fd {
                // Stays one-shot. Nothing ever reads the timerfd, so a
                // level-triggered registration would report it readable
                // forever once it expired and spin the loop.
                poller.add(
                    timer_fd.as_raw_fd(),
            ...
            // Same for the notifier: the eventfd reports readable only while
            // its counter is non-zero, and `clear` reads it back to zero.
            poller.add(
    ...
    pub fn wait_deadline(&self, events: &mut Events, deadline: Option<Instant>) -> io::Result<()> {
        ...
        if let Some(ref timer_fd) = self.timer_fd {
            // Setting the same timeout again is a syscall that changes
            // nothing, and a server waiting on its sockets asks for no
            // timeout on almost every turn. A timeout that has already
            // expired has to be set again, so only `None` is cached.
            // A zero timeout is passed straight to epoll_wait, and an
            // it_value of zero disarms a timerfd rather than firing it, so
            // zero and none are the same thing here.
            let arm = timeout.filter(|t| !t.is_zero());
            if arm.is_some() || self.timer_armed.load(std::sync::atomic::Ordering::Relaxed) {
                ...
                self.timer_armed
                    .store(arm.is_some(), std::sync::atomic::Ordering::Relaxed);

                // Only a timerfd that is actually set needs an interest. A
                // disarmed one cannot fire, so leaving it unregistered is
                // both correct and one syscall cheaper.
                if arm.is_some() {
                    self.modify(
                        timer_fd.as_fd(),
                        Event::readable(crate::NOTIFY_KEY),
        ...
        // Clear the notification, but only when one arrived. Reading an
        // eventfd that nobody wrote to is a syscall for nothing, and it is
        // the common case: most turns are woken by the sockets. The eventfd
        // is level-triggered and stays registered, so it does not have to be
        // re-armed either; `clear` reads its counter back to zero, which is
        // what stops it reporting readable.
        if events
            .list
            .iter()
            .any(|ev| ev.data.u64() as usize == crate::NOTIFY_KEY)
        {
            self.notifier.clear();
        }
        Ok(())
