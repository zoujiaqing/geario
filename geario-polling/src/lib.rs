...
impl Event {
    ...
    ///
    ///     let mut events = geario_polling::Events::new();
    ///
    ///     events.clear();
    ///     poller.wait(&mut events, None)?;
    ///
...
impl Poller {
    ...
    ///
    /// ```no_run
    /// use geario_polling::{Event, Poller};
    ///
    ...
    ///
    /// ```
    /// use geario_polling::{Events, Poller};
    ///
    /// let poller = Poller::new()?;
    ///
    /// // Notify the poller.
